...
const ALL_EVENTS = [
  ...
  },
  {
    value: "invoice.paid",
    label: "✅ Invoice Paid",
    desc: "When an invoice is marked paid",
  },
  {
    value: "invoice.overdue",
    label: "⚠️ Invoice Overdue",
    desc: "When an invoice becomes overdue",
  },
  {
    value: "invoice.deleted",
    label: "🗑️ Invoice Deleted",
    desc: "When an invoice is deleted",
  },
  {
    value: "customer.created",
    label: "👤 Customer Added",
    desc: "When a new customer is added",
  },
  {
    value: "customer.deleted",
    label: "👤 Customer Deleted",
    desc: "When a customer is removed",
  },
  {
    value: "payment.received",
    label: "💰 Payment Received",
    desc: "When a payment is recorded",
  },
];
    ...
    <div className="space-y-5 animate-fade-in max-w-2xl mx-auto">
      <div className="section-header">
        <div>
          ...
          <p className="text-dark-400 text-sm">
            Get notified when events happen in Trackeet
          </p>
        </div>
        <button
      ...
      <div className="card">
        ...
        <p className="text-sm text-dark-400 mb-4">
          When an event occurs in Trackeet, we'll send an HTTP POST request to
          your URL with the event data.
        </p>
        <div className="p-3 bg-dark rounded-xl font-mono text-xs overflow-x-auto">
          <p className="text-gray-400">
            // Example payload for invoice.created
          </p>
          <p className="text-success mt-1">{"{"}</p>
          <p className="text-warning ml-4">
            "event": <span className="text-white">"invoice.created"</span>,
          </p>
          <p className="text-warning ml-4">
            "timestamp":{" "}
            <span className="text-white">"2026-05-14T09:00:00.000Z"</span>,
          </p>
          <p className="text-warning ml-4">"data": {"{"}</p>
          <p className="text-warning ml-8">
            "invoiceNumber": <span className="text-white">"INV-2026-0001"</span>
            ,
          </p>
          <p className="text-warning ml-8">
            "totalAmount": <span className="text-white">15000</span>,
          </p>
          <p className="text-warning ml-8">
      ...
      <div className="card">
        ...
        {isLoading ? (
          ...
          <div className="empty-state py-8">
            ...
            <p className="text-dark-400 text-sm">
              Add a webhook to connect Trackeet to your systems
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            {webhooks.map((wh, i) => (
              <div
                key={i}
                className="p-4 rounded-xl border border-dark-200 dark:border-gray-700 bg-gray-50 dark:bg-dark"
              >
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1 text-xs text-dark-400">
                      ...
                      {wh.lastTriggered && (
                        <span>Last: {dayjs(wh.lastTriggered).fromNow()}</span>
                      )}
                    </div>
                  </div>
                    ...
                    <button
                      onClick={() =>
                        update({
                          ...
                          data: {
                            status:
                              wh.status === "active" ? "disabled" : "active",
                          },
                        })
                      }
                    >
                ...
                <div className="flex gap-1 flex-wrap">
                  {(wh.events || []).map((event) => (
                    <span
                      key={event}
                      className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-primary-light text-primary"
                    >
                      {event}
                    </span>
                  ))}
          ...
          <div className="bg-white dark:bg-surface rounded-3xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
                ...
                <div className="space-y-2">
                  {ALL_EVENTS.map((event) => (
                    ...
                    >
                      ...
                      <div>
                        <p
                          className={`text-sm font-semibold ${whEvents.includes(event.value) ? "text-primary" : "text-dark dark:text-white"}`}
                        >
                          {event.label}
                        </p>
              ...
              <button
                onClick={() => {
                  if (!whName.trim())
                    return toast.error("Enter a webhook name");
                  if (!whUrl.trim())
                    return toast.error("Enter an endpoint URL");
                  if (!whEvents.length)
              ...
              >
                {creating ? (
                  ...
                  <>
                    <Webhook size={16} /> Create Webhook
                  </>
                )}
              </button>
            </div>
